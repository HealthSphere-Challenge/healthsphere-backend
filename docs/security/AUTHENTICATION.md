...
Authenticated state changes (`PATCH /profile` and logout) require the session-bound CSRF header. GET, HEAD, and OPTIONS remain unaffected. Register and login are bootstrap endpoints without an authenticated session to bind; their JSON-only request bodies, explicit credentialed CORS policy, and `SameSite=Lax` cookies prevent ordinary cross-site form submission. The CSRF comparison uses `hmac.compare_digest`.

`last_used_at` advances on every successful authenticated request and is committed with that request transaction. Concurrent requests can commit nearby timestamps in either order, but the largest possible regression is their short overlap rather than the 24-hour idle window, so it cannot incorrectly expire an active session. Revocation and absolute expiry are checked independently on every request.
